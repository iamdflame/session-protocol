...
mod tests {
    ...
    /// Every day of 2025 through 2027, with its open and close or nulls.
    /// `tests/bell.test.ts` holds `bellTs` in sdk/src/bell.ts to it.
    #[test]
    fn emit_bell_vector() {
        let rows: Vec<String> = (day(2025, 1, 1)..day(2028, 1, 1))
            .map(|d| match (bell_ts(d, Kind::Open), bell_ts(d, Kind::Close)) {
                (Some(o), Some(c)) => format!("[{d},{o},{c}]"),
                _ => format!("[{d},null,null]"),
            })
            .collect();
        let json = format!(
            "{{\n \"note\": \"Generated by `cargo test -p session-bell emit_bell_vector`: rules::bell_ts for every day of 2025-2027.\",\n \"days\": [\n  {}\n ]\n}}\n",
            rows.join(",\n  ")
        );
        std::fs::create_dir_all("../../tests/vectors").ok();
        std::fs::write("../../tests/vectors/bells.json", json).unwrap();
    }
