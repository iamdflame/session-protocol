// Pyth's Lazer verifier, pyth-lazer-solana-contract 0.8.0
// (https://github.com/pyth-network/pyth-crosschain), Apache-2.0; see
// LICENSE-APACHE in this directory. Copied from the published crate with three
// changes, each marked `SESSION:` below, so SESSION's devnet bell can be
// verified by Pyth's own code while it holds a test signer instead of Pyth's:
//
//   1. a devnet program id of its own;
//   2. the `pyth-lazer-protocol` dependency dropped: verify_message reads one
//      constant from it (signature.rs), which is now written out;
//   3. `verify_ecdsa_message` removed, the one instruction that needed more
//      of that crate. A bell post uses Ed25519 only.
//
// verify_message and everything it calls are otherwise byte for byte Pyth's.
...
use {
    ...
    anchor_lang::{
        prelude::*,
        // SESSION: keccak and secp256k1_recover served only the ECDSA path (change 3).
...
// SESSION: `pyth_lazer_protocol as protocol` is no longer re-exported (change 2).
pub use crate::signature::{ed25519_program_args, Ed25519SignatureOffsets};

// SESSION: a devnet id of its own, and with it a storage PDA of its own; Pyth's
// STORAGE_ID constant and the test pinning it are Pyth's instance, not this one
// (change 1).
declare_id!("CVuKQFLc1PAuJ8y7kPckw8Hi8W6WhdeWrhM9UBVdm2qs");
...
pub mod pyth_lazer_solana_contract {
    ...
    // SESSION: verify_ecdsa_message removed (change 3).
