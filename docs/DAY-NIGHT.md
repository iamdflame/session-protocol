# DAY and NIGHT: where SESSION started

SESSION's first program splits a tokenized stock into the hours its price is
made and the hours it is not. It is still deployed and still settles on devnet,
and the research behind it is what led to bell orders: the night is where
tokenized stocks trade, and where they are least anchored to the real price.

This page is the README as it stood before the bell. Bell orders, the bell
oracle and the receipts are in the [README](../README.md),
[`BELL.md`](BELL.md) and [`CROSS.md`](CROSS.md).
...
| Event vault, no exchange session (PreStocks-shaped OPENAI) | [`FtpWQU…SGDDC`](https://explorer.solana.com/address/FtpWQUy4ZAoyBDS4zbwsLLc92qppciEgcCgCavVSGDDC?cluster=devnet) · [`/markets/OPENAI`](https://session-roan.vercel.app/markets/OPENAI) |
...
token is free to drift. Those are two different assets wearing one ticker, and
this program separates them into `X.NIGHT` and `X.DAY`.
...
- **Holding a real xStock needed a second fix, deeper than the first.** Anchor
  ...
  `GetAccountDataSize`. The story is below, because
